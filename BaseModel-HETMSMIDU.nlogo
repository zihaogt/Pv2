...
__includes ["Data.nls" "Table3.nls" "testing-frequency.nls" "manage-age-group.nls" "manage-undiagnosed.nls" "set-dropout-care.nls"
  ...
  "concurrency-HET-MSM.nls" "concurrency-IDU.nls" "verify-plot.nls" "write-output.nls" "old-write-output.nls"]
...
to sensitivity-analysis
  ...
  [
    set j 0
    repeat num-sensitive-range
    [
      initialize-output
      initialize-sensitive-globals

      set tempVal item i base-var-val * (1 + item j sensitive-range)
      ifelse tempVal >= 1
      [set assignVal 1]
      [
        ifelse tempVal <= 0
...
;  set num-HET-MEN (0.5 * (1 - (0.025 + 0.012 + 0.018)) * number-people * factor * 100000 / 441)
;  set num-HET-FEM (0.5 * (1 - (0.025 + 0.012 + 0.018)) * number-people * factor * 100000 / 441)
;  set num-MSM (0.05 * (1 - 0.07) * 0.5 * number-people * 100000 / 441) ;; assuming 2% of US population are MSM
;  set num-IDU-FEM (0.012 * number-people * 100000 / 441)
;  set num-IDU-MEN (0.018 * number-people * 100000 / 441)
;  set num-IDU-MSM (0.07 * 0.05 * 0.5 * number-people * 100000 / 441)
...
to go
  ...
  ;; discounting to year 2006, change proportions in care cascade components from 2012 to 2015 ;; first 15 years keeping at 2008 levels as a dry run
  ...
  [;; first 15 years dry-run, keep prevalence the same as 2006
    ...
    set partners-per-month n-values 8 [0]
    ...
    if (ticks - sim-dry-run) mod time-unit = 0 and ticks > t-start
    [set percent-needles-shared percent-needles-shared * percent-sharing-decline]
    ...
    ;; accumulating person-years for each continuum stages
    accum-person-years
    ...
    if (ticks - sim-dry-run) mod time-unit = 0 and ticks <= (sim-dry-run + time-unit * num-year-trans) ;; generarte transmissions for num-year-trans years
    [
      if ticks >= t-start
      [
        ;write-output
...
to dry-run-keep-prevalence
  ...
  [let currentPop people with [infected? = true and dead = 0]

    set num-HET-FEM count currentPop with [sex = 1] / item 0 prevalence
    set num-HET-MEN count currentPop with [sex = 2] / item 1 prevalence
    set num-MSM count currentPop with [sex = 3] / item 2 prevalence
    set num-IDU-FEM count currentPop with [sex = 4] / item 3 prevalence
    set num-IDU-MEN count currentPop with [sex = 5] / item 4 prevalence
    set num-IDU-MSM count currentPop with [sex = 6] / item 5 prevalence
...
to manage-proportion-care
  ...
  [set total-people-sex total-people with [sex = i]
    ...
    ifelse ticks >= (sim-dry-run + time-unit * (dry-run + 100)) ;year 2012 and future control no VLS according to times between rebound instead of using current proportions as some are due to lack of adherence. For future we want to assume 100% adherence
...
to acute-infection
  ask total-people with [age - age-at-infection <= 0.26 and stage < 2] ;; first 3 months of infection
    [if ticks > sim-dry-run + time-unit * 19 and ticks <= sim-dry-run + time-unit * 24 ;; individuals in acute phase are recently infected ones
      ...
      ;; counting transmission by stage, sex, and age
      ...
      set i 0
      let found 0
      while [i < num-age and found = 0]
      [if age < item i age-UB
...
to non-acute-infection
  ...
  ask total-people with [age - age-at-infection > 0.26 or stage >= 2]
    [set count-casuals item 2 counter-partner
...
to accum-person-years
  ...
  let i 0
  while [i < num-stage]
  [let people-stage people with [infected? = true and dead = 0 and stage = i + 1]
    set count-person-years-by-stage replace-item i count-person-years-by-stage (item i count-person-years-by-stage + count people-stage / time-unit)
    set i i + 1
  ]

  set i 0
  while [i < num-age]
  [let people-age people with [infected? = true and dead = 0 and age >= item i age-LB and age < item i age-UB]
    set count-person-years-by-age replace-item i count-person-years-by-age (item i count-person-years-by-age + count people-age / time-unit)
    set i i + 1
  ]

  set i 0
  while [i < num-sex]
  [let people-sex people with [infected? = true and dead = 0 and sex = i + 1]
    set count-person-years-by-sex replace-item i count-person-years-by-sex (item i count-person-years-by-sex + count people-sex / time-unit)
    set i i + 1
  ]
